...
| 1.136.5-7 | 04/09/2026 | Oficial | **Mais 3 rotas na API interna**: `POST /internal/hostel-memberships` (nova `add_user_to_hostel(user_id, hostel_id, role_name="Admin")` — vincula uma pessoa já existente a um hostel já existente, upsert idempotente via `ON CONFLICT(user_id, hostel_id) DO UPDATE`, cria a role "Admin" se ainda não existir; usada pra dar acesso do próprio login do usuário — `caiocfacosta@icloud.com` — à conta de demo `Teste2`, a pedido explícito pra poder entrar e ver, deixando claro que o mecanismo é genérico pra qualquer tipo de conta, não hostel-específico), `PATCH /internal/hostels/<id>/currency` (nova `set_hostel_currency`, upsert parcial só na coluna `settings.currency`, mesmo padrão de `routes/settings.py::update_settings` — corrigiu o dashboard de `Teste2` mostrando US$ numa conta brasileira que devia estar em R$), `GET /internal/hostels/<id>/guests` + `PATCH /internal/guests/<id>` (reaproveitam `get_guests_list`/`update_guest_profile` já existentes — ler e editar campos de contato/documento do hóspede sem sessão de login). |

| 1.137.0-6 | 05/09/2026 | Oficial | **Auditoria de confiabilidade do mapa de quartos** — usuário pediu revisão completa do fluxo de check-in/check-out/manutenção/limpeza depois de reportar "o mapa só pegou uma reserva" e citar uma falha anterior não detalhada num teste ao vivo com outro cliente (Miguel Seda), com o aviso "tem que estar redondo senão vai dar ruim". Investigação de `get_bed_map` confirmou que o sintoma reportado (Fernanda Lima não aparecendo como "Reservada") é comportamento intencional e já documentado no código: uma reserva só aparece como reservada no mapa quando `checkin_date <= hoje <= checkout_date` — reserva pra dali a dias não deve pintar a cama de reservada com antecedência (pedido de usuário anterior). **Dois bugs reais confirmados na auditoria, ambos em `database.py`**: (1) `checkin_reservation_to_bed` não impedia check-in duplicado da MESMA reserva em duas camas diferentes — a primeira cama ficava "ocupada" pra sempre, órfã, sem nenhuma reserva de fato apontando pra ela (nada a libera depois, já que o checkout só mexe na cama que está no `bed_id` ATUAL da reserva). (2) `checkout_reservation_bed` não verificava se a reserva já tinha sido finalizada — como o `bed_id` da reserva nunca é limpo após o checkout (fica lá de propósito, é o que `get_bed_map` usa pra mostrar "quem foi o último hóspede" numa cama suja/ocupada), um checkout DUPLICADO da mesma reserva (duplo clique, retry de rede, aba antiga aberta) reusava esse `bed_id` antigo e forçava a cama pra "precisa limpeza" — mesmo que, nesse meio tempo, outro hóspede JÁ tivesse feito check-in real naquela mesma cama. Esse é o tipo exato de bug que explicaria uma falha "mapa bugado" num teste ao vivo: a ação teria corrompido a ocupação de um hóspede sem nenhuma relação com a reserva que gerou o checkout duplicado. **Fix**: ambas as funções agora recusam a ação repetida com `ValueError` claro (checkin: "já está com check-in feito, faça checkout antes"; checkout: "já fez check-out"). Confirmado com teste isolado reproduzindo o cenário exato (reserva A → cama 1 → checkout → limpeza → reserva B → cama 1 → checkout DUPLICADO de A tentado de novo → bloqueado, cama 1 permanece `occupied` da reserva B intacta). **7 rotas novas em `routes/internal_api.py`** pra popular dado de demonstração usando o fluxo real (não atalho que só insere linha): check-in/check-out de reserva (reaproveitam as duas funções já corrigidas), criação de chamado de manutenção (reaproveita `create_maintenance_ticket` + `notify_on_duty_staff_for_ticket`, o mesmo caminho que a IA usa no chat — espelha em avisos/push de verdade), marcar cama limpa (`mark_bed_cleaned`, fecha o ciclo consumindo o kit de roupa de cama configurado), leitura de reservas de um hóspede e do mapa de camas (achar `reservation_id`/`bed_id` certos sem acesso direto ao banco), ajuste de plano de billing (`set_billing_plan` — necessário pra liberar o módulo de manutenção, que é gated por plano, numa conta de demo sem assinatura real), criar fornecedor/item de estoque (nova `create_inventory_item` extraída de `routes/inventory.py` pra `database.py`, mesmo padrão de `create_supplier_record` — a rota normal foi atualizada pra reaproveitar a mesma função, sem duplicar lógica) e definir kit de roupa de cama por tipo de cama (`set_linen_kit`). **Dado de demonstração populado em `Teste2`**: Marcos Andrade e Rafael Nogueira (hóspede novo) com check-in real em camas de verdade (ocupadas de verdade, não só reservadas) e perfil completo (telefone/email/endereço/nascimento/nacionalidade/documento) — a rota de criar reserva de demo também ganhou passagem de `phone`/`email`/`nationality` pro `create_reservation_record`, que já aceitava esses campos mas a rota não repassava (causa raiz de Marcos Andrade e Fernanda Lima terem nascido com perfil vazio nas versões anteriores). Dois chamados de manutenção reais na fila (cobertor extra pro Marcos, vazamento no banheiro pro Rafael), ambos gerados pelo caminho de produção completo (aparecem espelhados em notificação interna de plantão). Estoque populado (roupa de cama, higiene) com um item deliberadamente abaixo do mínimo com fornecedor vinculado (alerta de reposição de verdade, com mensagem sugerida) e kit de roupa de cama configurado pra cama `single`. Ciclo completo rodado com uma 3ª e 4ª reserva (Beatriz Carvalho, Juliana Prado) só pra gerar estado realista de limpeza: uma cama passou por check-in → checkout → limpeza (consumo real do kit, gerando "roupa suja" de verdade em `in_laundry_quantity`), outra ficou parada em "precisa limpeza" (fila de arrumação visível). Testado: cada endpoint novo primeiro contra SQLite isolado via Flask test client, deploy verificado por polling antes de cada ação seguinte contra produção (disciplina mantida desde o incidente de deploy da v1.133.0/v1.133.1) — 2 quedas passageiras de 502 do Render observadas e recuperadas sozinhas no meio do trabalho, sem relação com o código publicado. |

| 1.138.0-1 | 05/09/2026 | Oficial | **Hóspede vira Contato automaticamente no check-in real** — usuário viu a aba Contatos vazia numa conta de demo já com hóspede cadastrado e apontou "a StayFlow supostamente já teria a informação e criaria isso sozinha". Contatos era deliberadamente separado de Hóspedes desde a v1.116.0 (contato = alguém que a equipe salvou manualmente, pode nunca ter mandado mensagem; guest só existe depois de conversa/reserva real) — mudança de comportamento pedida é a direção OPOSTA do design original, e como afeta toda conta em produção (não só a de demo), a decisão foi explicada e confirmada com o usuário antes de implementar. Nova `sync_guest_to_contact(hostel_id, guest_id)` (`database.py`): só dispara no CHECK-IN de verdade (não em toda reserva/rascunho — reserva futura ainda não confirmada não devia virar contato) e só quando o hóspede tem TELEFONE (único identificador confiável pra dedupe, já que `contacts.phone` não tem `UNIQUE` constraint) — hóspede recorrente ou contato já cadastrado manualmente com o mesmo telefone nunca duplica, só reaproveita o registro existente. Ligada nos dois pontos reais de ocupação de cama: `checkin_reservation_to_bed` e `create_indefinite_stay` (quando já ocupa a cama na criação). 2 rotas novas na API interna: `GET /internal/hostels/<id>/contacts` (leitura) e `POST /internal/guests/<id>/sync-contact` (backfill manual — usado pra sincronizar retroativamente os 4 hóspedes de demo que já tinham feito check-in antes desse fix existir). Testado em SQLite isolado: contato criado no primeiro check-in com telefone; segundo check-in do MESMO hóspede (ciclo completo checkout→limpeza→nova reserva→check-in) não duplica; hóspede sem telefone nenhum não gera contato; check-in de um hóspede cujo telefone já tinha contato manual cadastrado reaproveita o registro existente (mantém o nome já salvo, não sobrescreve); estadia de longa duração sem `bed_id` (ainda não ocupa cama) não dispara o sync, com `bed_id` dispara normalmente. |

| 1.139.0 | 05/09/2026 | Oficial | **Pedido de reposição de estoque automático via WhatsApp** — usuário viu o alerta de estoque baixo (Sabonete, populado na v1.137.0-6) e perguntou por que só sugere mensagem pra copiar, sem um botão pra ela mesma identificar o fornecedor e mandar o pedido sozinha. `send_inventory_reorder_request(hostel_id, item_id)` (`database.py`) reaproveita `build_reorder_message` (já existente, antes só gerava texto sugerido) e `send_whatsapp_message` (`services/whatsapp_service.py`) usando o WhatsApp Business da PRÓPRIA hospedagem (mesmas credenciais do Embedded Signup, v1.53.0) pra mandar a mensagem direto pro telefone do fornecedor cadastrado — nunca finge que enviou: erro claro e específico se faltar fornecedor, se o fornecedor não tiver telefone, ou se a hospedagem não tiver WhatsApp conectado ainda. Nova coluna `inventory_items.reorder_requested_at` evita pedido duplicado em clique repetido e alimenta uma nota "pedido enviado em X" na UI. Botão "Pedir agora pelo WhatsApp" novo no card de alerta (`routes/inventory.py::request_inventory_reorder`, rota `POST /inventory/<id>/request-reorder`), só aparece quando o fornecedor tem telefone cadastrado. 5 chaves i18n novas em 11 idiomas (1.637→1.642), paridade confirmada via `tools/check_i18n_parity.py`. Testado em SQLite isolado com `send_whatsapp_message` mockado, cobrindo os 4 cenários: sem fornecedor (erro), fornecedor sem telefone (erro), sem WhatsApp conectado (erro), e envio bem-sucedido (mensagem correta, telefone normalizado só com dígitos, `reorder_requested_at` gravado). **Decisão deliberada de não testar contra produção**: o fornecedor cadastrado na conta de demo (`Teste2`) tem um telefone fictício, criado só pra popular o exemplo — mandar uma mensagem de WhatsApp de verdade pra esse número atingiria uma pessoa real ao acaso, então a validação ficou só no nível de lógica (SQLite + mock), sem disparo real. |

| 1.140.0-5 | 07/09/2026 | Oficial | **Auditoria de conversa da IA numa conta de imobiliária antes de conectar WhatsApp real** — usuário renomeou a conta de teste `Imobiliaria Teste` pra `Viana Soluções Imobiliárias` (cliente real, Elaine) e pediu garantia de que a IA "não pode travar, nem ficar dando volta que nem tonta" antes de instalar um número de WhatsApp brasileiro de verdade. Populados 3 imóveis pra VENDA (antes só havia 3 de locação) com dados completos (quartos/banheiros/m²/condomínio/IPTU/amenidades da lista fechada `REAL_ESTATE_AMENITIES`), moeda ajustada pra BRL. Nova `update_hostel_name()` (atualiza `hostels.name` E `settings.hostel_name` juntos — sem isso os dois ficam dessincronizados). Nova rota interna `POST /internal/hostels/<id>/simulate-message` (reaproveita `process_incoming_message`, o mesmo caminho dos webhooks reais, com `send_reply=False`) permitiu testar a IA de verdade — com tool calling real — sem nunca mandar mensagem de fato. Testados múltiplos cenários (alugar, comprar, agendar visita, financiamento inexistente, terreno inexistente, pergunta absurda de propósito, retomada de conversa) sem nenhum travamento/repetição/alucinação. **Bug real encontrado**: pedido "quinta que vem" numa segunda-feira foi agendado pra quinta da semana SEGUINTE (10 dias depois) em vez da mais próxima (3 dias depois), e a confirmação usava só a frase relativa, sem data explícita — risco real de cliente ir na visita no dia errado. Corrigido com nova instrução `_WEEKDAY_DATE_DISAMBIGUATION` (reaproveitada nos 3 prompts com data relativa: hospedagem, agência, software) — resolve dia da semana ambíguo pro mais próximo por padrão e sempre confirma com a data completa (dd/mm). Primeira versão da instrução só corrigiu a parte de "sempre confirmar com data explícita"; precisou de um exemplo concreto ancorado ("se hoje é segunda, quinta que vem = quinta DESSA MESMA semana") pra também corrigir a escolha da data errada — regra abstrata sozinha não bastou. Mais rotas internas de apoio à auditoria: listar itens de portfólio, listar/cancelar visitas de imóvel, listar contatos. Visitas de teste geradas durante a auditoria foram canceladas ao final, exceto uma (a que já rodou com o fix aplicado) deixada como exemplo ativo pro cliente ver na conta. |

| 1.141.0 | 07/09/2026 | Oficial | **Consolidação do catálogo de extras** — usuário percebeu, ao usar a conta de demonstração, que a aba Receitas também tinha opção de "adicionar passeio", e perguntou se não seria a mesma coisa que Serviços Extras (Portfólio) — sim, era: existiam DOIS catálogos de item vendável completamente separados desde antes desta sessão. Receitas tinha um mini-formulário (nome/tipo "Tour ou Upsell"/preço) gravando numa tabela antiga `offerings`, com sua própria ferramenta de IA (`get_addons`); Portfólio/"Serviços extras" tinha o catálogo completo (nome/descrição/categoria/preço/foto) gravando em `portfolio_items`, com a ferramenta `get_offerings`. O texto de exemplo dos dois convidava a cadastrar a MESMA coisa ("tours, late checkout, aluguel de toalha"), sem nenhuma ponte entre os dois — um dono de hospedagem cadastrando num lugar nunca veria o item aparecer no outro. **Auditoria antes de mexer**: nova `list_all_offerings_for_audit()` (temporária, removida depois de usada) confirmou que a tabela `offerings` estava vazia em TODOS os hostels — nenhum piloto real tinha usado essa tela, zero risco de perda de dado real na consolidação. Removidos: ferramenta `get_addons` e handler (`services/ai_service.py`), rotas `POST`/`DELETE /offerings` (`routes/revenue.py`), função `get_offerings_for_chat` (dead code). `get_revenue_summary` simplificada — Receitas volta a ser só o lado financeiro (oportunidades abertas + receita extra estimada), sem catálogo próprio. Parágrafo de "extras simples" do prompt (toalha/cobertor) fundido na seção única EXTRA SERVICES, que já usava `get_offerings`. Frontend: card de catálogo em Receitas agora mostra descrição curta + botão "Ver catálogo completo" (abre a aba Portfólio via `openPage('portfolio')`) + botão "➕ Adicionar" que abre o MESMO formulário completo de Portfólio (`openPortfolioItemModal()`) — pedido explícito do usuário pra não ter um formulário mais pobre em paralelo ("o mesmo formulário pode faltar informação? coloca formulário completo"). 2 chaves i18n novas em 11 idiomas (1.642→1.644), um erro de aspas retas dentro de string JS no bloco em chinês pego e corrigido antes de publicar (aspas retas trocadas por 「」). Testado: `get_revenue_summary` isolado confirma ausência da chave `offerings` e presença de `opportunities`/`extra_revenue`; `get_addons` confirmado fora do texto do prompt e da lista `RESERVATION_TOOLS`; paridade i18n e balanceamento de chaves/parênteses de todos os blocos `<script>`; app inteiro importado (368 rotas); conversa real testada via `simulate-message` nas DUAS contas (imobiliária listando os 3 imóveis de locação corretamente, hospedagem listando os 3 itens de cross-sell corretamente) depois do deploy, confirmando zero regressão na fusão das duas ferramentas de IA. |

| 1.141.1 | 07/09/2026 | Oficial | **Fix: opção "Assistente comercial da StayFlow" exposta a qualquer conta** — usuário notou, no próprio modal de WhatsApp da conta da imobiliária (Viana Soluções), que o dropdown "Este número responde como" mostrava a opção reservada da própria StayFlow pra qualquer cliente, sem gate nenhum. Risco real: `get_hostel_id_by_ai_persona` faz `LIMIT 1` — se um cliente selecionasse essa opção, o painel "Meu chat" da própria StayFlow podia passar a puxar a conversa ERRADA (de outro hostel) sem aviso nenhum. Corrigido nos dois lados: backend (`routes/settings.py`) só aceita `ai_persona='software'` se o e-mail de quem está logado estiver na allowlist `is_stayflow_admin_email` (mesmo critério de `require_stayflow_admin`); frontend esconde a opção do dropdown pra quem não é da equipe StayFlow (`window.STAYFLOW_SESSION.is_stayflow_admin`). Testado: usuário comum recebe 403 ao tentar via API direta, e-mail StayFlow consegue normalmente. |

| 1.142.0 | 07/09/2026 | Oficial | **WhatsApp Coexistence** — usuário pediu que o cliente pudesse continuar usando o app WhatsApp Business normal no celular ao MESMO tempo que a StayFlow atende pelo mesmo número, sem ser obrigado a migrar exclusivamente (motivado pela descoberta, no meio da tentativa de conectar o número da Viana Soluções, de que o fluxo padrão de Embedded Signup MIGRA o número — desconecta ele do app do celular). Pesquisa técnica completa feita contra a documentação OFICIAL da Meta (não um reseller reformatando — um fetch inicial via reseller trouxe um envelope de payload diferente do real, descartado depois de cruzar com a doc oficial) antes de escrever qualquer código, confirmando: o parâmetro que faltava no `FB.login()` (`extras.featureType: "whatsapp_business_app_onboarding"`, distinto do `extras.feature` já usado no fluxo padrão), o evento novo (`FINISH_WHATSAPP_BUSINESS_APP_ONBOARDING`), e o formato bruto dos 3 webhooks novos (`history`/`smb_app_state_sync`/`smb_message_echoes`, mesmo envelope `object/entry/changes/value` já usado por mensagem normal). **Construído**: (1) segundo botão "Conectar mantendo o WhatsApp Business no celular" no modal de Configurações → Comunicação — decide qual rota fecha a conexão pelo EVENTO recebido de verdade (não por qual botão foi clicado, já que a própria Meta pode deixar a pessoa mudar de ideia no meio do fluxo); (2) `exchange_whatsapp_coexistence_signup` (`services/meta_oauth_service.py`) NUNCA chama `/register` — diferença central vs o fluxo padrão, já que o número já está registrado de verdade no app do cliente, registrar de novo desconectaria ele — e dispara os dois pedidos de sincronização (`POST /{phone_number_id}/smb_app_data`, `sync_type` contato + histórico) exigidos pela Meta dentro de 24h da conexão, sob risco da conta ser desconectada; (3) `routes/whatsapp_webhook.py` passa a tratar os 3 tipos novos: `message_echoes` (mensagem mandada pela EQUIPE via app, espelhada como se fosse resposta da própria IA — sem isso a IA nunca saberia que o cliente já foi respondido manualmente e podia responder de novo ou dar informação desencontrada), `history` (histórico de até 180 dias, com `created_at` do timestamp REAL da mensagem, não "agora" — nova coluna opcional em `save_message_db_for_guest`), `state_sync` (contato do celular refletido na Agenda de Contatos, nova `sync_whatsapp_app_contact`, dedupe por telefone, nunca apaga contato cadastrado manualmente). Nova coluna `hostels.whatsapp_coexistence` marca o modo da conexão. 3 chaves i18n novas em 11 idiomas (1.644→1.647), paridade confirmada via `tools/check_i18n_parity.py`. Testado em SQLite isolado: os 3 tipos de webhook processados sem erro; hóspede deduplicado corretamente entre chamadas separadas (mesmo telefone via `message_echoes` e via `history`); papel da mensagem (user/assistant) resolvido certo comparando o remetente com o telefone do cliente; timestamp histórico preservado de verdade (confirmado gravado como 2023, não a data do teste); contato sincronizado sem duplicar. **Pendência que não é de código**: a feature só fica utilizável depois de 3 ações manuais do usuário no painel de desenvolvedor da Meta — (a) criar/confirmar a Configuration de Embedded Signup em v4 com "Onboard WhatsApp Business app users" habilitado, (b) configurar `WHATSAPP_CONFIG_ID` (e confirmar `META_APP_ID`/`META_APP_SECRET`) como variável de ambiente no Render — ainda ausente, é o motivo do botão de conexão automática nem aparecer hoje —, (c) assinar os 3 campos de webhook novos no painel Webhooks do app. |
